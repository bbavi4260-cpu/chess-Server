...
@app.route("/v1/home/<path:subpath>", methods=["GET"])
@app.route("/v1/homepage", methods=["GET"])
@app.route("/v1/dashboard", methods=["GET"])
@app.route("/v1/discover", methods=["GET"])
def home_subresource(subpath=""):
    """Keep optional home modules from breaking the whole home screen."""
    return jsonify({
        "code": 0,
        "status": "ok",
        "success": True,
        "data": {"sections": [], "items": [], "cards": [], "games": [], "notifications": []},
        "sections": [],
        "items": [],
        "cards": [],
        "games": [],
...
def bot_personalities():
    # Some APK builds parse this home response as a BotPersonalities object,
    # not as a raw List. A raw top-level array causes:
    # "Expected BEGIN_OBJECT but was BEGIN_ARRAY at path $".
    personalities = [
