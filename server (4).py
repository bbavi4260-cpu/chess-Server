...
def cometd_engine():
    ...
    for msg in data:
        ...
        if channel == "/meta/handshake":
            ...
            COMETD_CLIENTS[client_id] = {"created_at": time.time(), "last_seen": time.time()}
            response.append({
                "id": msg_id,
            ...
            # Bayeux clients send repeated connects as a heartbeat. Keep the
            # same clientId instead of creating a new connection each time.
            if not client_id:
                client_id = f"client_{uuid.uuid4().hex[:12]}"
            client = COMETD_CLIENTS.setdefault(
                client_id, {"created_at": time.time(), "last_seen": time.time()}
            )
            ...
            COMETD_CLIENTS.pop(client_id, None)
            response.append({
                "id": msg_id,
                "channel": channel,
                "successful": True,
...
def live_websocket(ws, game_id=None):
    ...
    while True:
        ...
        try:
            ...
            if action in {"ping", "heartbeat", "keepalive"}:
                ws.send(json.dumps({"event": "pong", "timestamp": int(time.time() * 1000)}))
            elif action == "move":
                ...
                if move_uci:
                    ...
                    if move in board.legal_moves:
                        board.push(move)
                        ws.send(json.dumps({
                            "event": "move_played", "game_id": game_id, "fen": board.fen(),
                            "timestamp": int(time.time() * 1000)
                        }))
                    else:
                        ws.send(json.dumps({"event": "error", "message": "Illegal move"}))
            elif action in {"status", "connection_status"}:
                ws.send(json.dumps({
                    "event": "status", "status": "connected", "game_id": game_id,
                    "fen": board.fen(), "timestamp": int(time.time() * 1000)
                }))
